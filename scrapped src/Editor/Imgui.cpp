...
#include "Editor.h"
#include <IO/IO.h>
void Imgui::Init(Vulkan_Instance &vkinstance, LogicalDevice &device, Queue &queue, RenderPass &rpp, LWindow &window,
                 Format format)
{
    ...
    std::string font_path =
        IO::GetAssets().GetFolder("editor").GetFolder("fonts").GetFile("JetBrainsMono-Regular.ttf").GetPath();
...
void Imgui::PushEditorTools()
{
    ...
    imguiGroup->PushObject<EditorLambda>(
        0, "Imgui Demo", [&]() { ImGui::MenuItem("Imgui Demo", 0, &ShowImguiDemo); },
        [&]() {
            if (ShowImguiDemo)
                ImGui::ShowDemoWindow();
        });
    imguiGroup->PushObject<EditorLambda>(
        1, "Implot Demo", [&]() { ImGui::MenuItem("Implot Demo", 0, &ShowImplotDemo); },
        [&]() {
            if (ShowImplotDemo)
