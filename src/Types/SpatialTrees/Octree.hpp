...
#include <pch.h>
template <uint32 TreeDepth, typename LeafData = void, typename BranchData = void> class Octree
{

    public:
    using MortonIndexType = std::conditional<TreeDepth <= 10,uint32,uint64>::type;
    using VoxelPosition = uint32;
    using LeafDataType = LeafData;
    private:
    constexpr static inline uint32 MAX_DEPTH = 32; // given 2^32 is the max a uint32 coord can store
    struct Node
    {
        std::bitset<8> ChildrenSet = 0;
        // std::array<MortonIndexType, 8> children = {0};
        LeafData data;
    ...
    {
        auto operator()(MortonIndexType const &x) const noexcept -> uint64_t
        {
            return x;
        }
    };
    //using NodesContainer = ankerl::unordered_dense::map<MortonIndexType, Node>;
    //  using NodesContainer = ankerl::unordered_dense::segmented_map<MortonIndexType,Node,custom_hash_simple>;
    using NodesContainer = std::unordered_map<MortonIndexType, Node>;
    ...
    MortonIndexType IndexFromChildPos(uvec3 ChildPos, uint32 ChildDepth)
    {
        return Morton::SIMD::MortonEncodeBounded<3>(ChildPos / SizeOfNodeAtDepth(ChildDepth),
                                                    OctreeResolution(ChildDepth));
    }
  ...
  public:
    ...
    /// @brief Gets the node that exists at a specified level and Morton Index
    /// @param level
    /// @param index
    /// @return
    Node &AtMortonIndex(uint32 level, MortonIndexType index);
    ...
    constexpr uint32 OctreeResolution(uint32 AtDepth = TreeDepth) const
    {
    ...
    constexpr size_t AbsMaxMemoryUse() const
    {
        ...
        for (int i = 0; i < TreeDepth; i++)
        {
            const uint32 res = OctreeResolution(i);
...
inline Octree<TreeDepth, LeafData, BranchData>::Octree()
{
...
template <uint32 TreeDepth, typename LeafData, typename BranchData>
inline Octree<TreeDepth, LeafData, BranchData>::Node &Octree<TreeDepth, LeafData, BranchData>::AtPositionLeaf(
    uvec3 Position)
{
...
template <uint32 TreeDepth, typename LeafData, typename BranchData>
inline Octree<TreeDepth, LeafData, BranchData>::Node &Octree<TreeDepth, LeafData, BranchData>::AtMortonIndex(
    uint32 level, MortonIndexType index)
{
    return Levels[level][index];
}

template <uint32 TreeDepth, typename LeafData, typename BranchData>
inline Octree<TreeDepth, LeafData, BranchData>::Node &Octree<TreeDepth, LeafData, BranchData>::InsertLeaf(
    uvec3 GlobalPosition)
{
    LASSERT(glm::all(glm::greaterThanEqual(GlobalPosition, uvec3(0))) &&
                glm::all(glm::lessThan(GlobalPosition, uvec3(OctreeResolution()))),
            "Invalid Position");
    ...
    for (int idepth = 0; idepth < TreeDepth; idepth++)
    {
        ...
        const MortonIndexType EncodedChildPosition = IndexFromChildPos(PositionOfChild, idepth + 1);
...
template <uint32 TreeDepth, typename LeafData, typename BranchData>
inline void Octree<TreeDepth, LeafData, BranchData>::InsertLeafs(const std::vector<uvec3> &positions)
{
    static std::vector<std::pair<uvec3, MortonIndexType>> posMorton;
    ...
    {
        MortonIndexType idx = Morton::SIMD::MortonEncode<3>(pos); // implement MortonEncode(x,y,z)
        // posMorton[i] = {pos, idx};
...
template <uint32 TreeDepth, typename LeafData, typename BranchData>
inline void Octree<TreeDepth, LeafData, BranchData>::ForEach(std::function<void(const NodeData &)> func)
{
    struct NodeInfo
    {
        uvec3 NodePos;
        MortonIndexType MortonIndex;
        uint32 Depth;
        Node *node;
        bool isLeaf = false;
    };
    ...
    while (!stack.empty())
    {
        ...
        for (uint8 i = 0; i < 8; i++)
        {
            if (node->HasChildAtIndex(i))
            {
                ...
                //std::cerr << "Fetching node at depth " << ChildInfo.Depth << " index: " << ChildInfo.MortonIndexType<< " Position " << to_string(ChildInfo.NodePos) << std::endl;
