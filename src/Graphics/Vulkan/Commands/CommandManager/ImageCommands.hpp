...
struct CopyImageCommand : Command
{
    ...
    void Execute(GPURef<VK::CommandBuffer> cmdBuffer) override
    {
        ...
        cmdBuffer->GetHandle().copyImage(std::holds_alternative<GPURef<Graphics::Texture>>(SrcImage)
...
struct ClearColorImageCommand : Command
{
    ...
    ClearColorImageCommand(std::variant<GPURef<Graphics::Texture>, vk::Image> TargetImage, ImageLayouts TargetImageLayout,
                    vec4 ClearColor)
