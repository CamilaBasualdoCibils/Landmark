...
struct BindPipelineCommand : Command
{
    ...
    {
        cmdBuffer->GetHandle().bindPipeline((vk::PipelineBindPoint)BindPoint, *pipeline);
    }
};
...
struct PushConstantsCommand : Command
{
    GPURef<VK::PipelineLayout> pipelineLayout;
    size_t offset;
    Flags<VK::ShaderStages> Stages;
    std::vector<ValueType> values;
    PushConstantsCommand(GPURef<VK::PipelineLayout> pipelineLayout, size_t offset, Flags<VK::ShaderStages> Stages,
                         std::vector<ValueType> values)
        : pipelineLayout(pipelineLayout), offset(offset), Stages(Stages), values(std::move(values))
    {
    }
    ...
    void Execute(GPURef<VK::CommandBuffer> cmdBuffer) override
    {
        cmdBuffer->GetHandle().pushConstants(*pipelineLayout, (vk::ShaderStageFlags)*Stages, offset,values.size() * sizeof(ValueType),values.data());
    }
};
} // namespace VK