...
public:
    ...
    template<typename T>
    bool has() const { return _storage.contains(typeid(T)); }

    template<typename T>
    [[nodiscard]] T *try_get() { return has<T>() ? &get<T>() : nullptr; }

    template<typename T>
    [[nodiscard]] const T *try_get() const { return has<T>() ? &get<T>() : nullptr; }
