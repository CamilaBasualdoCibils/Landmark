...
VULKAN_SDK="libvulkan-dev vulkan-tools mesa-utils"

apt-get update
DEBIAN_FRONTEND=noninteractive apt-get -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install -y build-essential curl gdb clang clangd clang-tools zip unzip tar git cmake ${GLEW_DEPS} ${GLFW_DEPS} ${VULKAN_DEPS} ${VULKAN_SDK}
...
# === Nsight Graphics ===
chmod +x ./tools/Nsight-Graphics.sh